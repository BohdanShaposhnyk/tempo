...
export class TelegramConfigService {
  ...
  constructor() {
    ...
    if (!encryptionSecret) {
      this.logger.warn(
        'ENCRYPTION_SECRET not set, using default (not secure for production)',
      );
    }
  }
  ...
  private getEncryptionKey(): Buffer {
    const secret =
      process.env.ENCRYPTION_SECRET || 'default-secret-key-not-secure';
  ...
  async setTelegramConfig(botToken: string, chatId: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to encrypt Telegram config: ${getErrorMessage(error)}`,
      );
  ...
  async getTelegramConfig(): Promise<TelegramConfig> {
    if (
      !this.encryptedBotToken ||
      !this.encryptedChatId ||
      !this.iv ||
      !this.botTokenTag ||
      !this.chatIdTag
    ) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to decrypt Telegram config: ${getErrorMessage(error)}`,
      );
