...
export class ThornodeService {
  ...
  async getTransactionStatus(txHash: string): Promise<ThornodeTxStatus | null> {
    try {
        ...
        this.httpService.get<ThornodeTxStatus>(url).pipe(
          ...
          catchError((error: AxiosError) => {
            if (error.response) {
              ...
              this.logger.warn(
                `THORNode API request failed for tx ${txHash}: ${error.message}`,
              );
            } else {
              this.logger.warn(
                `THORNode API error for tx ${txHash}: ${error.message}`,
              );
            }
            return of(null);
          }),
      ...
      this.logger.debug(
        `Successfully fetched transaction status for ${txHash}`,
      );
      return response.data;
    } catch (error: unknown) {
      this.logger.error(
        `Failed to fetch transaction status for ${txHash}: ${getErrorMessage(
          error,
        )}`,
      );
      return null;
