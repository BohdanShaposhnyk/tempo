...
import {
  formatAmount,
  getStreamSwapSizeInUSD,
  calculateSizeFromThornodeTx,
...
export class DetectorService implements OnApplicationBootstrap {
  ...
  private getInputAmountAndSizeInUSD(
    action: MidgardAction,
    txStatus: ThornodeTxStatus | null,
  ): { $size: number; inputAmount: string } {
    ...
    return {
      $size: getStreamSwapSizeInUSD(action),
  ...
  async handleActionDetected(event: {
    ...
    try {
      ...
      if (isStreamSwap) {
        ...
        const txHash = action.in[0].txID;
        await this.handleStreamSwap(txHash, action, event.height);
      }
    } catch (error: unknown) {
      this.logger.error(
        `Error processing action at height ${event.height}: ${getErrorMessage(
          error,
        )}`,
      );
    }
  ...
  private async handleStreamSwap(
    ...
    try {
      ...
      const { $size, inputAmount } = this.getInputAmountAndSizeInUSD(
        action,
        txStatus,
      );
      ...
      this.logger.error(
        `Error handling stream swap: ${getErrorMessage(error)}`,
      );
    }
  }
  ...
  handleStreamSwapDetected(event: StreamSwapDetectedEvent): void {
    ...
    this.logger.log(
      `Valid opportunity spotted: size=${formatAmount($size)}$, duration=${estimatedDurationSeconds}s, txHash=${opportunity.txHash}`,
    );
