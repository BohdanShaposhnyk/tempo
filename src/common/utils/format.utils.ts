import {
  MidgardAction,
  ThornodeTxStatus,
} from 'src/modules/thorchain/interfaces/thorchain.interface';
 ...
 */
export function calculateSizeFromThornodeTx(
  txStatus: ThornodeTxStatus,
...
export function getStreamSwapSizeInUSD(action: MidgardAction): number {
  try {
    const {
      swap: { streamingSwapMeta, outPriceUSD = '0', inPriceUSD = '0' } = {},
    } = action.metadata ?? {};
    const { outEstimation, depositedCoin } = streamingSwapMeta ?? {};

    const [inAmount, outAmount] = [
      depositedCoin?.amount ?? '0',
      outEstimation ?? '0',
    ].map((x) => formatAmount(x));

    return Math.max(
      inAmount * parseFloat(inPriceUSD),
      outAmount * parseFloat(outPriceUSD),
    );
  } catch {
    return 0;
  }
